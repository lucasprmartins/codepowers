...
**Anunciar no início:** "Estou usando a skill finish-dev para concluir este trabalho."
...
Execute o agente `codepowers:check` para verificação de:
...
Use a ferramenta `AskUserQuestion` para perguntar ao usuário o ID da task relacionada a esta PR. Exemplo de pergunta: "Qual o ID da task relacionada a esta PR? (ex: 82)". Use o valor para compor o título no formato `[TASK-<id>] <tipo>: <descrição>`.
